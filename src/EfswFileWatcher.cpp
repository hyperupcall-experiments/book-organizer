...
EfswFileWatcher::EfswFileWatcher(const std::string& watchDirectory)
    : watchDir(watchDirectory) {
    fileWatcher = std::make_unique<efsw::FileWatcher>();
}

EfswFileWatcher::~EfswFileWatcher() {
    stop();
}

void EfswFileWatcher::start() {
    if (running) {
        return;
    }
...
void EfswFileWatcher::setCallback(FileChangeCallback callback) {
    changeCallback = callback;
}

std::vector<std::filesystem::path> EfswFileWatcher::getCurrentFiles() const {
...
void EfswFileWatcher::handleFileAction(efsw::WatchID watchid, const std::string& dir,
...
void EfswFileWatcher::scanDirectory() {
    if (!std::filesystem::exists(watchDir)) {
        return;
    }
...
bool EfswFileWatcher::isBookFile(const std::filesystem::path& path) const {
